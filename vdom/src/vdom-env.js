// vdom-env.js
// not mean to be loaded directly! see vdom.js

/* repackaged for use with vdom.js
 *   By Tim Dedischew <http://github.com/timjs/>
 * Copyright 2009 Tim Dedischew, under same license as above / env.js
*/
...
// scoped wrapper
// given an object (possibly VM's global), load it up with env-js
function createENV(win){
  win.obj_nodes = new java.util.HashMap();
  win.events = [{}];
  win.timers = [];
  ...
  with(win) {
	...
	win.document = null; // placeholder for scope dereference

	// Browser Navigator

	win.navigator = {
	  get userAgent(){
		return "Mozilla/5.0 (env.js) Gecko/2000000 Firefox/2.0.0.0";
	  }
	};
	
	var curLocation = (new java.io.File("./")).toURL();
	
	win.__defineSetter__("location", function(url){
						   if (url == 'about:blank') {
							 win.document = new DOMDocument("<html><head><title>about:blank</title></head><body></body></html>");
	...
	win.__defineGetter__("location", function(url){
							  return {
								get protocol(){
	...
	win.clearInterval = function(num){
	  if ( timers[num] ) {
		timers[num].stop();
		delete timers[num];
	  }
	...
	DOMDocument.prototype = {
	  ...
	  // added getter for document.firstChild -tjd 2009.06
	  get firstChild(){
									   ...
									   get tagName(){
										 ...
										 get outerHTML(){
										   ...
										   get attributes(){
											 ...
											 /* try to do something useful for compound style attribute... */
											 var a = [];
											 for (var p in this.style)
											   if (this.style[p]+'' != "undefined")
												 a.push(p+":"+this.style[p]);
											 if (a.length)
											   attr['style'] = a.join(";");
											 if (attr['defaultValue'] == '') delete attr['defaultValue'];
											 /* -- tjd 2009.06 */
																							 ...
																							 function execScripts( node ) {
																							   if ( node.nodeName == "SCRIPT" ) {
																								 if ( !node.getAttribute("src") ) {
																								   eval.call( win, node.textContent );
																								 }
																							   } else {
		...
		function makeRequest(){
		  ...
		  function handleResponse(){
			...
			if ( self.responseText.match(/^\s*</) ) {
			  ...
			  } catch(e) { throw new Error('setting responseXML error: '+e); }
			}
		  }
